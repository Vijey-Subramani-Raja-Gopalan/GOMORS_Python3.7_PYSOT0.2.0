...
from itertools import chain #tvh
...
class MoSyncStrategyNoConstraints(BaseStrategy):
    ...
    def log_completion(self, record):
        ...
        # tvh-------------------------------
        # reformat array values out of record.params
        #
        '''f = list(record.params)
        oned_f = chain.from_iterable(f)
        record.params = tuple(oned_f)'''
        # ----------------------------------
    ...
    def save_plot(self, i):
        ...
        maxgen = (self.maxeval - self.numinit)/(self.nsamples*self.ncenters)
        curgen = (self.numeval - self.numinit)/(self.nsamples*self.ncenters) + 1
        # if self.data.pf is not None:   #tvh
            # plt.plot(self.data.pf[:,0], self.data.pf[:,1], 'g')   #tvh
        all_fvals = [rec.fx for rec in self.evals]
        all_fvals = np.asarray(all_fvals)
        # plt.plot(all_fvals[:,0], all_fvals[:,1], 'k+')   #tvh
        # if fvals.ndim > 1:   #tvh
            # plt.plot(fvals[:,0], fvals[:,1], 'b*')   #tvh
        # plt.title(title)   #tvh
        #plt.draw()
        #plt.savefig('Final')        #tvh
        # plt.clf()   #tvh
