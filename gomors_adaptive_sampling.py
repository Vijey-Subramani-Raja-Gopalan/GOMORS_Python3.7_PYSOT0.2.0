...
class MultiSampling(object):
    ...
    def __init__(self, strategy_list, cycle):
        if cycle is None:
            cycle = np.array(list(range(len(strategy_list))))
...
class EvolutionaryAlgorithm(object):
    ...
    def __generate_cand__(self, xfront, xgap=None):
        if xgap is None:
            ...
            problem = GlobalProblem(self.data.dim, self.data.nobj, self.fhat, self.data.lb, self.data.ub) #tvh
            generator = CustomGenerator(500)
            generator.create(problem, xfront)
            #algorithm = NSGAIII(problem, divisions_outer=100,generator=generator)
            algorithm = NSGAII(problem=problem, population_size=500, generator=generator)
            ...
            problem = GapProblem(self.data.dim, self.data.nobj, self.fhat, xgap, rgap, self.data.lb, self.data.ub) #tvh
            ...
            algorithm = NSGAII(problem=problem, population_size=500) #tvh
        algorithm.run(2500)
    ...
    def make_points(self, npts, xbest, xfront, front, subset=None, proj_fun=None):
        ...
        if self.epsilons is None:
            ...
            print('GOMORS non-dominated elements and number of targets: ' + str(self.fhvals_nd.shape))
        ...
        self.previous_points = np.vstack((self.previous_points,
                                          np.asmatrix(xnew)))
        # xnew = new candidate points for evaluation
        # fh_new = approximated values of new candidate points for evaluation (based on meta model)
        # self.fhvals_nd = approximated values of the approximated pareto frontier (based on meta model)
