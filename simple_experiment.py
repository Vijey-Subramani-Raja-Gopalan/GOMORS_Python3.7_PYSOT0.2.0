...
def main():
    ...
    nthreads = 4  # number of parallel used cores - more than 4 are not tested by Taimoor
    #maxeval = 500
    maxeval = 512  #max no. of original function evaluations
    nsamples = 4        # number of expensive evaluated points picked from metamodel-optimization results
    ...
    #data = DTLZ4()
    data = ZDT3(nobj=2)
    #data = Schwefel_with_Sphere_Problem(nobj=2)
    print(data.dim)
    num = 1
    epsilons = [0.05, 0.05]	# for Epsilon-Selection (instead of HyperVolumeSelection) of each objective value - in this case 0.05 for each objective
